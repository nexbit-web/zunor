...
Третій шар — **компоненти**, окремим vitest-проєктом у jsdom: рунні стори
(бейджі, список чатів, відкат оптимістичних оновлень), картки налаштувань,
стрічка сповіщень, бульбашка чату, картка замовлення, профілі й розбір
мікроформату відповідей асистента. Тут же друга лінія головної обіцянки:
**телефон не з'являється в розмітці, навіть якщо приїхав у пропсах**.

Усі тести — в одній теці `src/tests/` (`npm run test`), справжня БД для них
не потрібна. Не покриті: форми входу й онбордингу, чат цілком, стрічки
заявок, лендинг і вендорений `components/ui/**`.
