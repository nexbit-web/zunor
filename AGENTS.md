...
Тут уже дві діри одного кореня (детект мови й страховка фото-чіпсів у
`agent.ts`). У JavaScript «слово» для `\b` і `\w` — це `[A-Za-z0-9_]`, тому
в суцільно кириличному рядку **межі слова немає ніде**, а `\w*` завжди
матчить порожній рядок. Регекс виглядає робочим, мовчки не спрацьовує
ЖОДНОГО разу, і помітити це можна лише тестом.

Пишеш регекс по українському чи російському тексту — бери Unicode-класи:

```ts
/(^|[^\p{L}])(привіт|треба)(?![\p{L}])/giu   // замість \b…\b
