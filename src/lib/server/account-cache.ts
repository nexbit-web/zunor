...
/**
 * 5 хвилин. Це верхня межа затримки ЛИШЕ для змін, зроблених повз
 * застосунок — наприклад, бан руками в БД. Усе, що робить сам застосунок
 * (онбординг, апгрейд ролі, бан, верифікація пошти), скидає кеш явно через
 * invalidateAccount і діє миттєво; CRM робить те саме через Pusher-подію
 * (див. subscribeToInvalidations нижче).
 *
 * Було 10 секунд — при тому, що коментарі поруч обіцяли 30. На тарифі, де
 * база тарифікується за часом роботи, короткий TTL означав запит до неї
 * практично на кожній навігації дашборда заради полів, які майже ніколи
 * не змінюються.
 */
