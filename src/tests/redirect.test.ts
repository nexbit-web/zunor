...
// табуляцію, \n і \r з URL ще ДО розбору. Тому "/<TAB>/evil.com"
// перетворюється на "//evil.com" — protocol-relative, тобто чужий домен.
// Фільтр же дивиться на сирий рядок і цих трьох символів не бачить.
//
// Робоче посилання: /user/login?redirectTo=/%09/evil.com
...
describe('керуючі символи всередині шляху', () => {
  ...
  for (const [label, ch] of controls) {
    it(`ДІРКА: ${label} на початку шляху виводить на чужий домен`, () => {
      expect(safeRedirectTarget(`/${ch}/evil.com`)).toBe(FALLBACK)
    })
  }

  it('ДІРКА: підсумок — вихід фільтра лишається на нашому origin', () => {
    for (const [, ch] of controls) {
